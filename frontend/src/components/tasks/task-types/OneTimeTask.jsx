...
import { useCompleteOneTimeTask, useUncompleteOneTimeTask } from "../../../hooks/update-task.js";

export function OneTimeTask({task}) {
    const completeTask = useCompleteOneTimeTask();
    const uncompleteTask = useUncompleteOneTimeTask();
    return (
        <TaskBox>
            ...
            <button type="button" 
                    onClick={() => task.completed ? uncompleteTask.mutate({taskTag: task.tag}) : completeTask.mutate({taskTag: task.tag})} 
